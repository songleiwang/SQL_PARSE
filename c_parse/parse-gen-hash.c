...
typedef struct hash_lex_struct hash_lex_struct;
...
struct hash_lex_struct
{
    ...
    gchar   last_char;
    union{
            struct hash_lex_struct *char_tails;
            gint                    iresult;
    };
    gint    ithis;
};
...
hash_lex_struct *get_hash_struct_by_len(hash_lex_struct **root_by_len, 
                                        gint len, gint *max_len)
{

    if (*max_len<len){
        *root_by_len = (hash_lex_struct *)ut_realloc((gchar*)*root_by_len, 
...
void insert_into_hash(hash_lex_struct *root, const gchar *name, 
                      gint len_from_begin, gint index, gint function)
{
    ...
    if (root->first_char == -1) {
        gint index2 = root->iresult;
        const gchar *name2 = (index2 < 0 ? sql_functions[-index2-1] :
                             symbols[index2]).name + len_from_begin;
        root->first_char = (gint) ((guchar)name2[0]);
        root->last_char = (gchar) root->first_char;
        tails= (hash_lex_struct*)ut_malloc(sizeof(hash_lex_struct));
        root->char_tails = tails;
        tails->first_char = -1;
        tails->iresult = index2;
    }

    gsize real_size= (root->last_char-root->first_char+1);

    if (root->first_char>(*name)) {
        gsize new_size = root->last_char-(*name)+1;
        if (new_size<real_size) g_printf("error!!!!\n");
        tails = root->char_tails;
        tails = (hash_lex_struct*)ut_realloc((gchar*)tails, 
                                           sizeof(hash_lex_struct)*new_size);
        root->char_tails = tails;
        g_memmove(tails+(new_size-real_size),tails,
                  real_size*sizeof(hash_lex_struct));
        end = tails + new_size - real_size;
        for (cur = tails; cur<end; cur++)
            cur->first_char = 0;
        root->first_char = (gint) (guchar) *name;
  }

    if (root->last_char<(*name)) {
        gsize new_size = (*name)-root->first_char+1;
        if (new_size<real_size) g_printf("error!!!!\n");
        tails = root->char_tails;
        tails = (hash_lex_struct*)ut_realloc((gchar*)tails, 
                                           sizeof(hash_lex_struct)*new_size);
...
void insert_symbols()
{
    ...
    for (cur = symbols; i<array_elements(symbols); cur++, i++){
        hash_lex_struct *root = get_hash_struct_by_len(&root_by_len,
                                                      cur->length,&max_len);
...
void insert_sql_functions()
{
    gint    i = 0;
    SYMBOL *cur;
    for (cur = sql_functions; i < (gint) array_elements(sql_functions); cur++, i++) {
        hash_lex_struct *root = get_hash_struct_by_len(&root_by_len,
                                                      cur->length,&max_len);
...
void calc_length()
{
    ...
    for (cur = symbols; cur < end; cur++)
        cur->length = (guchar) strlen(cur->name);
...
void add_struct_to_map(hash_lex_struct *st)
{
    ...
    size_hash_map += 4;
    hash_map = (gchar*)ut_realloc((gchar*)hash_map,size_hash_map);
    hash_map[size_hash_map-4] = (gchar) (st->first_char == -1 ? 0 :
                                        st->first_char);
    hash_map[size_hash_map-3] = (gchar) (st->first_char == -1 ||
                                      st->first_char == 0 ? 0 : st->last_char);
    if (st->first_char == -1) {
        hash_map[size_hash_map-2] = ((guint)(gint16)st->iresult)&255;
        hash_map[size_hash_map-1] = ((guint)(gint16)st->iresult)>>8;
    }else if (st->first_char == 0) {
        hash_map[size_hash_map-2] = ((guint)(gint16)array_elements(symbols))&255;
        hash_map[size_hash_map-1] = ((guint)(gint16)array_elements(symbols))>>8;
    }
}


void add_structs_to_map(hash_lex_struct *st, gint len)
{
...
void set_links(hash_lex_struct *st, gint len)
{
    hash_lex_struct *cur, *end= st+len;
    for (cur = st; cur<end; cur++) {
        if (cur->first_char != 0 && cur->first_char != -1) {
        gint ilink = cur->char_tails->ithis;
...
void print_hash_map(const gchar *name)
{
    gchar  *cur;
    gint    i;

    g_printf("static guchar %s[%d]= {\n",name,size_hash_map);
...
int main(gint argc,gchar **argv)
{
    ...
    /* Broken up to indicate that it's not advice to you, gentle reader. */
    g_printf("/* Do " "not " "edit " "this " "file!  This is generated by "
             "gen_lex_hash.cc\nthat seeks for a perfect hash function */\n\n");
    g_printf("#include \"parse-symbol.h\"\n\n");
    ...
    g_printf("\nstatic guint sql_functions_max_len=%d;\n", max_len);
    g_printf("\nstatic guint symbols_max_len=%d;\n\n", max_len2);

    g_printf("\
SYMBOL *get_hash_symbol(const gchar *s,\n\
                               guint len,gboolean function)\n\
{\n\
    register guchar         *hash_map;\n\
    register const gchar    *cur_str = s;\n\
\n\
    if (len == 0) {\n\
        return(NULL);\n\
    }\n"
  ...
  g_printf("\
    if (function){\n\
        ...
        register guint32 cur_struct = uint4korr(hash_map+((len-1)*4));\n\
\n\
        for (;;){\n\
            register guchar first_char = (guchar)cur_struct;\n\
\n\
            if (first_char == 0) {\n\
                register gint16 ires = (gint16)(cur_struct>>16);\n\
                ...
                register guint count = (guint) (cur_str - s);\n\
                return lex_casecmp(cur_str,res->name+count,len-count) ? 0 : res;\n\
            }\n\
\n\
            register guchar cur_char = (guchar)to_upper_lex[(guchar)*cur_str];\n\
            if (cur_char < first_char)\n\
                return 0;\n\
            cur_struct >>= 8;\n\
            if (cur_char>(guchar)cur_struct)\n\
                return 0;\n\
\n\
            cur_struct >>= 8;\n\
            cur_struct = uint4korr(hash_map+\n\
                                 (((guint16)cur_struct + cur_char - first_char)*4));\n\
            cur_str++;\n\
        }\n"
);
